...
% 	%COMPONENTS 2 AND 3: gamma or inverse gamma
    mmm=3%10;%20;%(the mean of component)
    vvv=10;%(the variance of the component)
	if MM =='GGM' 
         %mmm=30;%(the mean of component)
         %vvv=1;%(the variance of the component)
         
         %xx=0.001:0.001:100;plot(xx,gampdf(xx,alphaGm(mmm,vvv),betaGm(mmm,vvv)));hold on; plot(xx,ig(xx,alphaIG(mmm,vvv),betaIG(mmm,vvv)),'r')
